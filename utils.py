...
def data_csv(root, split):
    ...
    for i in img:
        img_path = i.rstrip()
        img_path1 = os.path.basename(img_path)
        lbl_path = os.path.join(label_base, img_path1.split('_')[0],
    ...
    a = root.split('/')[-2]
    data.to_csv(a + split[:-1] + '_data.csv', index=False)


########################################################################################
# def test_result_csv(root, split):
#     img_base = os.path.join(root, 'leftImg8bit', split)
#     img = []
#     for folder in os.listdir(img_base):
#         for file in os.listdir(os.path.join(img_base + folder + '/')):
...
#     data = pd.DataFrame(data=d)
#
#     data.to_csv('images/test/' + split[:-1] + '_result.csv', index=False)


########################################################################################
# def save_image_pth(save_pth):
#     img_base = save_pth
#     img = []
#     for folder in os.listdir(img_base):
#     # for file in os.listdir(os.path.join(img_base + folder + '/')):
#     #     img.append(img_base + folder + '/' + file)
#         img = os.listdir(os.path.join(img_base + folder + '/'))
#     return img


########################################################################################
def read_image_pth(save_pth):
    img_base = save_pth
    img = []
    for folder in os.listdir(img_base):
        for file in os.listdir(os.path.join(img_base + folder + '/')):
...
def conv2D_shape(input_shape, kernel, stride, padding=(0, 0), dilation=(1, 1)):
    ...
    '''
    # h_in, w_in = np.reshape(input_shape,[512,1024])
