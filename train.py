...
def validation(model, test_loader, criterion):
    ...
    for batch_num, (feats, labels) in enumerate(test_loader):
        ...
        # del labels
    model.train()
    Validation_Loss = sum(avg_loss) / len(avg_loss)
    Mean_IOU = Visualize.iou(temp, labels.detach().cpu().numpy())
    Avg_Mean_IOU = sum(avg_IOU) / len(avg_IOU)
    print('Validation Loss: {:.4f}'.format(Validation_Loss))
    print(f'Mean IOU: {Mean_IOU}')
    print(f'Avg. Mean IOU: {Avg_Mean_IOU:.4f}')

    del feats
    # del temp
    del labels

    return np.array(inp), np.array(temp), Validation_Loss, Mean_IOU, Avg_Mean_IOU
